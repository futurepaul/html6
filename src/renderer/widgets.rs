    ...
    match node {
        ...
        Node::Paragraph { children } => {
            ...
            if children.len() == 1 && matches!(&children[0], Node::Image { .. }) {
                return build_widget(&children[0]);
            }

            // Handle inline formatting by building widgets for each child
            if children.iter().any(|c| matches!(c, Node::Strong { .. } | Node::Emphasis { .. } | Node::Link { .. })) {
                ...
                for (i, child) in children.iter().enumerate() {
                    ...
                    flex = flex.with_child(build_inline_widget(child, add_space));
                }
                NewWidget::new(flex)
            } else {
                let text = render_children_to_text(children);
        ...
        Node::VStack { children, width, height, flex: flex_val, align } => {
            ...
            for child in children {
                if let Some(child_flex) = get_child_flex(child) {
                    flex_widget = flex_widget.with_flex_child(build_widget(child), child_flex);
                } else {
                    flex_widget = flex_widget.with_child(build_widget(child));
                }
            }
        ...
        // Components that need data - render placeholder for now
        Node::Expr { expression } => {
            wrap_in_flex(NewWidget::new(Label::new(format!("{{{}}}", expression))))
        }
