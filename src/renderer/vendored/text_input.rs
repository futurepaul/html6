...
//
// Vendored from Masonry to fix hardcoded white focus border (line 298-301 in original)
// TODO: Upstream PR to Masonry
...
use masonry::core::{
    ...
    Properties, PropertiesMut, PropertiesRef, Property, RegisterCtx, Update, UpdateCtx, Widget, WidgetId,
    WidgetMut, WidgetPod, HasProperty,
};
...
use masonry::vello::kurbo::{Affine, Point, Rect, Size};
...
// Custom property for focused border color
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct FocusedBorderColor {
    pub color: Color,
}

impl Property for FocusedBorderColor {
    fn static_default() -> &'static Self {
        static DEFAULT: FocusedBorderColor = FocusedBorderColor {
...
impl Widget for TextInput {
    ...
    fn property_changed(&mut self, ctx: &mut UpdateCtx<'_>, property_type: TypeId) {
        ...
        // Handle FocusedBorderColor changes
        if property_type == TypeId::of::<FocusedBorderColor>() {
    ...
    fn paint(&mut self, ctx: &mut PaintCtx<'_>, props: &PropertiesRef<'_>, scene: &mut Scene) {
        ...
        // Use FocusedBorderColor when focused, otherwise use BorderColor
        let border_color = if ctx.has_focus_target() {
            props.get::<FocusedBorderColor>().color
        } else {
            props.get::<BorderColor>().color
        };
        ...
        stroke(scene, &border_rect, border_color, border_width.width);
    }
